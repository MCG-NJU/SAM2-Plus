...
def save_ann_png_with_box(path, mask, box_xywh, palette):
...
def save_masks_and_boxes_to_dir(
    output_mask_dir,
    video_name,
    frame_name,
    per_obj_output_mask,
    ...
    assert len(per_obj_output_mask.keys()) == 1 and list(per_obj_output_mask.keys())[0] == 1, "Only support single object now"
    assert len(per_obj_output_box_xywh.keys()) == 1 and list(per_obj_output_box_xywh.keys())[0] == 1, "Only support single object now"
    ...
    if not per_obj_png_file:
        ...
        output_box_xywh = per_obj_output_box_xywh[1].squeeze()
        save_ann_png_with_box(output_mask_path, output_mask, output_box_xywh, output_palette)
    else:
        for object_id, object_mask in per_obj_output_mask.items():
            ...
            output_box_xywh = per_obj_output_box_xywh[object_id].squeeze()
            output_mask_path = os.path.join(
...
def save_boxes_to_dir(
    video_boxes_xywh,
    output_bbox_dir,
    video_name,
):
    if "/" in video_name:
        video_name_sub_dir = video_name.split("/")[0]
        os.makedirs(os.path.join(output_bbox_dir, video_name_sub_dir), exist_ok=True)
    os.makedirs(output_bbox_dir, exist_ok=True)

    bbox_list = []
    for frame_id, object_box_xywh in sorted(video_boxes_xywh.items(), key=lambda x: x[0]):
        assert len(object_box_xywh.keys()) == 1 and list(object_box_xywh.keys())[0] == 1, "Only support single object now"
        bbox_list.append(object_box_xywh[1])
    bbox_list = np.concatenate(bbox_list)
    file_path = os.path.join(output_bbox_dir, video_name + ".txt")
    np.savetxt(file_path, bbox_list, fmt="%d")
...
def sot_inference(
    ...
    for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(
        ...
        per_obj_output_box_xywh = {
            out_obj_id: np_box_clamp_xywh(np_box_xyxy_to_xywh(np_masks_to_boxes(per_obj_output_mask[out_obj_id])))
            for i, out_obj_id in enumerate(out_obj_ids)
        }

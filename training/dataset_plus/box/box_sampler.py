...
from training.dataset.vos_sampler import MAX_RETRIES, SampledFramesAndObjects, VOSSampler
...
class RandomUniformSampler(VOSSampler):
    ...
    def sample(self, video, segment_loader, epoch=None):
        ...
        for retry in range(MAX_RETRIES):
            ...
            loaded_boxes_xyxy, loaded_visibles, loaded_pngs = segment_loader.load(frames[0].frame_idx)
            ...
            for object_id, visible in loaded_visibles.items():
                if visible:
                    visible_object_ids.append(object_id)
            ...
            if retry >= MAX_RETRIES - 1:
                raise Exception(f"No visible objects, from video {video.video_name} at frame {frames[0].frame_idx}")

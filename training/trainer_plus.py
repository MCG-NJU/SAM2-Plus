...
from training.utils_plus.logger import Logger_plus
...
class Trainer_plus:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        self._peft_model(peft)  # Apply peft to the model
        self._construct_optimizers()
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None, merge_peft=False):
        ...
        if self.peft_config is not None and merge_peft:
            logging.info("Merging PEFT model before saving checkpoint. Only Once!")
            self.model = unwrap_ddp_if_wrapped(self.model)
            if 'image_encoder' in self.peft_config:
                if not self.model.separate_image_encoder:
                    self.model.image_encoder = self.model.image_encoder.merge_and_unload()
                else:
                    assert not hasattr(self.model, 'image_encoder')
                    self.model.image_encoder_mask = self.model.image_encoder_mask.merge_and_unload()
                    self.model.image_encoder_point = self.model.image_encoder_point.merge_and_unload()
                    self.model.image_encoder_box = self.model.image_encoder_box.merge_and_unload()
            if 'memory_attention' in self.peft_config:
                if not self.model.separate_memory_attention:
                    self.model.memory_attention = self.model.memory_attention.merge_and_unload()
                else:
                    assert not hasattr(self.model, 'memory_attention')
                    self.model.memory_attention_mask = self.model.memory_attention_mask.merge_and_unload()
                    self.model.memory_attention_point = self.model.memory_attention_point.merge_and_unload()
                    self.model.memory_attention_box = self.model.memory_attention_box.merge_and_unload()
    ...
    def _step(
        ...
        key = batch.metadata.task  # key for dataset
        if key == "mask":
            loss = self.loss[key](outs_batch=outputs, targets_batch=batch.masks)
        elif key == "point":
            loss = self.loss[key](outs_batch=outputs, target_masks_batch=batch.masks, target_points_batch=batch.points, target_visibles_batch=batch.visibles)
        elif key == 'box':
            loss = self.loss[key](outs_batch=outputs, target_masks_batch=batch.masks, target_boxes_batch=batch.boxes, target_visibles_batch=batch.visibles)
        else:
        ...
        if self.steps[phase] % self.logging_conf.log_visual_frequency == 0:
            T = len(outputs)
            self.logger.log_visualize_multi_task_io(
                task=key, step=self.steps[phase], phase = phase,
                input_imgs=batch.data.img_batch,
                
                gt_segments=batch.data.masks,
                gt_boxes=batch.data.boxes if key == 'box' else None,
                gt_points=batch.data.points if key == 'point' else None,
                
                output_segments=torch.stack([outputs[i]['pred_masks_high_res'] for i in range(T)]),
                output_boxes=torch.stack([outputs[i]['pred_boxes_xyxy'] for i in range(T)]) if key == 'box' else None,
                output_points=torch.stack([outputs[i]['pred_points_xy'] for i in range(T)]) if key == 'point' and 'pred_points_xy' in outputs[0] else None,
                
                prompt_points=[outputs[i]['point_inputs'] for i in range(T)],
                prompt_masks=[outputs[i]['mask_inputs'] for i in range(T)],

                obj_to_frame_idx=batch.data.obj_to_frame_idx,
            )
    ...
    def run_train(self):
        ...
        if self.epoch == self.max_epochs:
            self.save_checkpoint(self.epoch, ['checkpoint_last'], merge_peft=False)
            self.save_checkpoint(self.epoch, ['checkpoint_last_merge_peft'], merge_peft=True)
    ...
    def _run_step(
        ...
        self.scaler.scale(loss).backward()  # [name for name, param in self.model.named_parameters() if param.grad is not None]
        loss_mts[loss_key].update(loss.item(), batch_size)
    ...
    def _setup_components(self):
        ...
        self.logger = Logger_plus(self.logging_conf)
    ...
    def _peft_model(self, peft):
        """
        Apply peft only to image_encoder.
        """
        if peft is None:
            self.peft_config = None
            return
        
        self.peft_config = {k: instantiate(peft[k]) for k in peft.keys()}
        assert all([isinstance(v, LoraConfig) for (k, v) in self.peft_config.items()])
        
        if 'image_encoder' in self.peft_config:
            if not self.model.separate_image_encoder:
                self.model.image_encoder = get_peft_model(self.model.image_encoder, self.peft_config['image_encoder'])
            else:
                assert not hasattr(self.model, 'image_encoder')
                self.model.image_encoder_mask = get_peft_model(self.model.image_encoder_mask, self.peft_config['image_encoder'])
                self.model.image_encoder_point = get_peft_model(self.model.image_encoder_point, self.peft_config['image_encoder'])
                self.model.image_encoder_box = get_peft_model(self.model.image_encoder_box, self.peft_config['image_encoder'])
        if 'memory_attention' in self.peft_config:
            if not self.model.separate_memory_attention:
                self.model.memory_attention = get_peft_model(self.model.memory_attention, self.peft_config['memory_attention'])
            else:
                assert not hasattr(self.model, 'memory_attention')
                self.model.memory_attention_mask = get_peft_model(self.model.memory_attention_mask, self.peft_config['memory_attention'])
                self.model.memory_attention_point = get_peft_model(self.model.memory_attention_point, self.peft_config['memory_attention'])
                self.model.memory_attention_box = get_peft_model(self.model.memory_attention_box, self.peft_config['memory_attention'])

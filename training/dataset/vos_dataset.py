...
class VOSDataset(VisionDataset):
    ...
    def _get_datapoint(self, idx):
        ...
        for retry in range(MAX_RETRIES):
            try:
                ...
                if not self._check_first_frame_objs_all_exist(datapoint):
                    raise Exception(
                        f"Empty segment found in datapoint {idx} after transforms. The data path is {sampled_frms_and_objs.frames[0].image_path}"
                    )
    ...
    def _check_first_frame_objs_all_exist(self, datapoint):
        return all([torch.any(obj.segment) for obj in datapoint.frames[0].objects])

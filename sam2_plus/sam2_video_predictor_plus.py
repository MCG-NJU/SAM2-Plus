...
from sam2_plus.modeling.sam2_base import NO_OBJ_SCORE, SAM2Plus
from sam2.utils.misc import concat_points, fill_holes_in_mask_scores, load_video_frames
from training.dataset_plus.point.utils import generate_gaussian


class SAM2VideoPredictor_Plus(SAM2Plus):
    ...
    def init_state(
        ...
        images, video_height, video_width = load_video_frames(
            video_path=video_path, frame_names=frame_names,
    ...
    @classmethod
    def from_pretrained(cls, model_id: str, **kwargs) -> "SAM2VideoPredictor_Plus":
        """
    ...
    # Merge self.add_new_mask and self.add_new_points_or_box
    # 1) del `point_inputs_per_frame.pop(frame_idx, None)`
    # 2) del `point_inputs_per_frame.pop(frame_idx, None)`
    def add_new_points_and_generate_gaussian_mask(
        self,
        inference_state,
        frame_idx,
        obj_id,
        ...
        """Add new points and mask to a frame."""
        ...
        if box is not None:
            raise ValueError("box is not supported in this method")
        ...
        mask = generate_gaussian(points.squeeze(0).squeeze(0), img_size=(self.image_size, self.image_size), radius=radius, sigma=sigma)

        if not isinstance(mask, torch.Tensor):
            mask = torch.tensor(mask, dtype=torch.float) # mask = torch.tensor(mask, dtype=torch.bool)
        assert mask.dim() == 2
        ...
        if mask_H != self.image_size or mask_W != self.image_size:
            raise ValueError("generated mask size should be the same as the model's image size")
        ...
        _, video_res_masks, video_res_boxes_xyxy = self._get_orig_video_res_output(
            inference_state, consolidated_out["pred_masks_video_res"], consolidated_out["pred_boxes_xyxy_norm_video_res"]
        )
        return frame_idx, obj_ids, video_res_masks, video_res_boxes_xyxy
    ...
    def _get_orig_video_res_output(self, inference_state, any_res_masks, any_res_boxes_xyxy_norm):
        """
        ...
        any_res_boxes_xyxy_norm = any_res_boxes_xyxy_norm.to(device, non_blocking=True) if any_res_boxes_xyxy_norm is not None else None
        ...
        # Only unnorm the pred boxes here.
        video_res_boxes_xyxy = any_res_boxes_xyxy_norm * torch.tensor([video_W, video_H, video_W, video_H], device=device) if self.task == "box" else None
        if self.non_overlap_masks:
            video_res_masks = self._apply_non_overlapping_constraints(video_res_masks)
        return any_res_masks, video_res_masks, video_res_boxes_xyxy
    ...
    def _consolidate_temp_output_across_obj(
        ...
        if consolidate_at_video_res:
            ...
            consolidated_box_key = "pred_boxes_xyxy_norm_video_res"
        else:
            consolidated_H = consolidated_W = self.image_size // 4
            consolidated_mask_key = "pred_masks"
            consolidated_box_key = "pred_boxes_xyxy_norm"
        ...
        consolidated_out = {
            ...
            consolidated_box_key: torch.full(
                size=(batch_size, 1, 4),
                fill_value=0,
                dtype=torch.float32,
                device=inference_state["storage_device"],
        ...
        for obj_idx in range(batch_size):
            ...
            obj_box_xyxy_norm = out["pred_boxes_xyxy_norm"]
            consolidated_pred_masks = consolidated_out[consolidated_mask_key]
            consolidated_pred_boxes_xyxy_norm = consolidated_out[consolidated_box_key]
            if obj_mask.shape[-2:] == consolidated_pred_masks.shape[-2:]:
                consolidated_pred_masks[obj_idx : obj_idx + 1] = obj_mask
                if self.task == "box":
                    consolidated_pred_boxes_xyxy_norm[obj_idx : obj_idx + 1] = obj_box_xyxy_norm
            else:
                ...
                if self.task == "box":
                    consolidated_pred_boxes_xyxy_norm[obj_idx : obj_idx + 1] = obj_box_xyxy_norm    # only unnorm box in self._get_orig_video_res_output(), not here
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                ...
                if frame_idx in obj_output_dict["cond_frame_outputs"]:
                    ...
                    pred_boxes_xyxy_norm = current_out["pred_boxes_xyxy_norm"].to(device, non_blocking=True) if self.task == "box" else None
                    pred_object_score_logits = current_out["object_score_logits"].to(device, non_blocking=True)
                    if self.clear_non_cond_mem_around_input:
                        # clear non-conditioning memory of the surrounding frames
                        self._clear_obj_non_cond_mem_around_input(
                            inference_state, frame_idx, obj_idx
                    ...
                    current_out, pred_masks, pred_boxes_xyxy_norm = self._run_single_frame_inference(
                        inference_state=inference_state,
            ...
            if len(pred_masks_per_obj) > 1:
                ...
                all_pred_boxes_xyxy_norm = torch.cat(pred_boxes_xyxy_norm_per_obj, dim=0) if self.task == "box" else None
                all_pred_object_score_logits = torch.cat(pred_object_score_logits_per_obj, dim=0)
            else:
                all_pred_masks = pred_masks_per_obj[0]
                all_pred_boxes_xyxy_norm = pred_boxes_xyxy_norm_per_obj[0] if self.task == "box" else None
                all_pred_object_score_logits = pred_object_score_logits_per_obj[0]
            _, video_res_masks, video_res_boxes_xyxy = self._get_orig_video_res_output(
                inference_state, all_pred_masks, all_pred_boxes_xyxy_norm
            )
            video_pred_object_score_logits = all_pred_object_score_logits
            yield frame_idx, obj_ids, video_res_masks, video_res_boxes_xyxy, video_pred_object_score_logits
    ...
    def _run_single_frame_inference(
        ...
        pred_boxes_xyxy_norm_gpu = current_out["pred_boxes_xyxy_norm"]
        ...
        pred_boxes_xyxy_norm = pred_boxes_xyxy_norm_gpu.to(storage_device, non_blocking=True) if self.task == "box" else None
        ...
        compact_current_out = {
            ...
            "pred_boxes_xyxy_norm": pred_boxes_xyxy_norm,
            "obj_ptr": obj_ptr,
            "object_score_logits": object_score_logits,
        }
        return compact_current_out, pred_masks_gpu, pred_boxes_xyxy_norm_gpu
...
class SAM2VideoPredictorVOS_Plus(SAM2VideoPredictor_Plus):
    ...
    def _compile_all_components(self):
        ...
        self.sam_unified_decoder.forward = torch.compile(
            self.sam_unified_decoder.forward,
            mode="max-autotune",
            fullgraph=True,
            dynamic=False,  # Accuracy regression on True
        )
    ...
    def forward_image(self, img_batch: torch.Tensor):
        """
        Identical to the corresponding method in the parent (SAM2VideoPredictor_Plus), but
    ...
    def _forward_sam_heads(
        ...
        boxes_xyxy_norm_multiboxes = boxes_xyxy_norm_multiboxes.clone() if boxes_xyxy_norm_multiboxes is not None else None
        ious = ious.clone()
